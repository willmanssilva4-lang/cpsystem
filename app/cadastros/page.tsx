...
export default function RegistrationsPage() {
  ...
  const renderCardContent = (option: typeof REGISTRATION_OPTIONS[0], index: number) => (
    ...
    >
      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl sm:rounded-3xl bg-brand-bg flex items-center justify-center text-brand-blue border border-brand-border group-hover:bg-brand-blue group-hover:text-white transition-all duration-300 shadow-sm shrink-0">
        <option.icon className="w-5 h-5 sm:w-6 sm:h-6" strokeWidth={2.5} />
      </div>
      <div>
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 sm:space-y-8 bg-brand-bg/50 min-h-screen relative overflow-x-hidden">
      ...
      <div className="flex flex-col gap-2 relative z-10">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl sm:rounded-[2rem] bg-brand-blue/10 flex items-center justify-center text-brand-blue border border-brand-blue/20 shadow-inner shrink-0">
            <LayoutGrid className="w-7 h-7 sm:w-8 sm:h-8" />
          </div>
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-brand-text-main italic uppercase">Central de Cadastros</h1>
            <p className="text-brand-text-sec font-medium text-xs sm:text-sm mt-1 max-w-xl">Gerencie as configurações operacionais, equipe, estoque e parceiros do seu sistema em um único painel consolidado.</p>
          </div>
        </div>
      </div>
        ...
        {REGISTRATION_OPTIONS.map((option, index) => (
          option.label === 'Clientes' ? (
            <button
              ...
              className="group flex flex-col items-start gap-4 p-5 sm:p-6 md:p-8 rounded-3xl sm:rounded-[2.5rem] border border-brand-border bg-brand-card hover:border-brand-blue/30 hover:shadow-2xl hover:shadow-brand-blue/5 hover:-translate-y-2 transition-all duration-300 text-left active:scale-[0.98] w-full"
            >
